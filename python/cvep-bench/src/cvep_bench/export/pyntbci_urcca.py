...
def main() -> None:
    ...
    x_test, y_test = chronological_test_split(x, y, args.folds, args.fold_index)
    online_pred = []
            ...
            state_snapshots.append(
                {
                    ...
                    "avg_x": np.asarray(cca.avg_x_)
                    .reshape(-1)
                    .astype(np.float32)
                    .tolist(),
                    "avg_y": np.asarray(cca.avg_y_)
                    .reshape(-1)
                    .astype(np.float32)
                    .tolist(),
    ...
    metadata = {
        ...
        "pyntbci_online_accuracy": class_accuracy(y_test, online_pred),
    }
    np.savez_compressed(
        args.output,
